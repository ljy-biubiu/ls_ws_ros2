...
namespace lslidar_ch_driver
{
    ...
    LslidarChDriver::LslidarChDriver(
        const rclcpp::NodeOptions &options) : rclcpp::Node("lslidar_node", options),
                                              socket_id(-1),
    ...
    bool LslidarChDriver::createRosIO()
    {
        ...
        difop_thread_ = std::make_shared<std::thread>([this]()
                                                      { difopPoll(); });
        return true;
    }
    ...
    void LslidarChDriver::publishPointCloud()
    {
        ...
        auto elapsed2 = std::chrono::duration_cast<std::chrono::milliseconds>(end_2 - start_2); // 计算执行时间
        std::cout << "pub need time ms:" << elapsed2.count() << std::endl;
    ...
    int LslidarChDriver::convertCoordinate(struct Firing lidardata)
    {
        ...
        else if (lidar_type == "ch64w")
        {
            ...
            if (line_num / 4 % 2 == 0)
            {
                ...
                double xita_hangle_new = -3.6636 * pow(10, -7) * pow(xita_hangle, 3) + 5.2766 * pow(10, -5) * pow(xita_hangle, 2) + 0.9885 * pow(xita_hangle, 1) + 0.5894;
            ...
            else
            {
                ...
                double xita_hangle = acos(cos_xita_F) * RAD_TO_DEG;
                double xita_hangle_new = -3.6636 * pow(10, -7) * pow(xita_hangle, 3) + 1.4507 * pow(10, -4) * pow(xita_hangle, 2) + 0.9719 * pow(xita_hangle, 1) + 1.9003;
                cos_xita_F = cos(xita_hangle_new * DEG_TO_RAD);
                sin_xita_F = sin(xita_hangle_new * DEG_TO_RAD);
                add_distance = -0.017;
            }
