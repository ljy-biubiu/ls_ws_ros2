...
void PaintArea::paintEvent(QPaintEvent *event)
{
    ...
    // else if (true)
    {
        // painter1.setPen(QPen(​QColor(255,0,0));//设置画笔的颜色/点云的颜色
        std::cout << " xCloud->size()"<< xCloud->size()<< std::endl;
        ...
        pen.setColor(QColor(r, g, b, 30));
        painter1.setPen(pen);

        for (size_t j = 0; j < xCloud->size(); j++)
        {
            if (xCloud->points[j].z > area[0].Area_height_top || xCloud->points[j].z < area[0].Area_height_down)
            {
                continue;
            }
...
void PaintArea::drawArea(QPainter &painter, int index)
{
    ...
    { // 绘制区域
        ...
        {
            painter.setPen(QPen(Qt::blue, 2 / zoom));
        }
